    ...
    if ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        if (hash('sha256', $Password) === $storedHashedPassword) {
            ...
            if (!empty($userPicturePath) && file_exists($userPicturePath)) {
                $userPicturePath = 'http://asianabpolymar.com/abpolymer/' . $userPicturePath;
            } else {
                $userPicturePath = 'http://asianabpolymar.com/abpolymer/default_profile_picture.png';
            }
...
function get_moneyreceipts_users($conn, $UserID) {
    ...
    // define the list of admin or special users who can see all records
    $adminUsers = [2879]; // you can add more user IDs here if needed

    if (in_array($UserID, $adminUsers)) {  
        // Admin user – can see all receipts
        $sql = "SELECT 
            ROW_NUMBER() OVER (ORDER BY MRID) AS SL,
            MRID,
            CONCAT(MRNo, ' - Tk. ', FORMAT(ISNULL(AmountReceived, 0), 'N2')) AS MRInfo1,
        ...
        // Normal user – can see only their own receipts
        $sql = "SELECT 
            ROW_NUMBER() OVER (ORDER BY MRID) AS SL,
            MRID,
            CONCAT(MRNo, ' - Tk. ', FORMAT(ISNULL(AmountReceived, 0), 'N2')) AS MRInfo1,
...
function get_salesorders_users($conn, $UserID) {
    ...
    $full_access_users = [ 1759, 1997, 1985,];

    if (in_array($UserID, $full_access_users )) {
...
//     FROM [dbo].[sndSalesOrders] 
// where
// 		 OrderTypeID = 1 and  Status in (1,2) and userid in (select userid from sndUsers where ReportingToUserID = ?)
// 	 And (select count(*) from sndSalesOrderDetails sod where sod.SalesOrderID = sndSalesOrders.SalesOrderID)>0
//   or OrderTypeID = 1 and  Status in (1,2) and userid in (select userid from sndUsers where ReportingToUserID in (select userid from sndUsers where ReportingToUserID = ?))
//     And (select count(*) from sndSalesOrderDetails sod where sod.SalesOrderID = sndSalesOrders.SalesOrderID)>0
//   order by SalesOrderID desc";
...
function get_salesordersApproval($conn, $UserID) {
    ...
    // Define admin users
    $admin_users = [1997, 1759]; // Add all admin user IDs
    
    $is_admin = in_array($UserID, $admin_users);
    ...
    -- User Authorization Status
    CASE 
        WHEN " . ($is_admin ? "1=1" : "0=1") . " THEN 'Authrized'
        WHEN AppStatus = (
            SELECT TOP 1 AppStatus FROM sndApprovals 
            WHERE RoleID IN (SELECT RoleID FROM sndUserRoleMapping WHERE UserID = ?) 
            AND ApprovalTables = 'sndSalesOrders' 
    ...
    if ($is_admin) {
        // Admin users see ALL orders - no hierarchy restriction
        $sql .= " ORDER BY SalesOrderID DESC";
        $params = [$UserID, $UserID, $UserID, $UserID];
    } else {
        // Regular users see only orders in their hierarchy
        $sql .= " AND (
            userid in (select userid from sndUsers where ReportingToUserID = ?)
            OR 
            userid in (select userid from sndUsers where ReportingToUserID in (select userid from sndUsers where ReportingToUserID = ?))
        )
        ORDER BY SalesOrderID DESC";
        $params = [$UserID, $UserID, $UserID, $UserID, $UserID, $UserID];
    }
...
//     // Define admin users who have full approval access
//     $admin_users = [1997, 1985, 1759]; // Add your admin user IDs here
...
//     // Prepare parameters
//     if ($is_admin) {
//         // Admin users don't need the role/district checks in WHERE clause
//         $params = [$UserID, $UserID, $UserID, $UserID];
//     } else {
//         $params = [$UserID, $UserID, $UserID, $UserID, $UserID, $UserID];
//     }
...
function get_VisitEntryApprovalSum($conn, $UserID) {
    ...
    // === CASE 1: Top-level / Admin users (see all) ===
    if (in_array($UserID, [1])) {  // <-- put your real admin IDs here
        $sql = "
        SELECT 
            ROW_NUMBER() OVER (ORDER BY COUNT(CASE WHEN VP.AppStatus = 1 THEN VP.VisitPlanID END) DESC) AS SL,
            VP.VisitUserID,
            U.EmpName AS VisitUserName,
            COUNT(CASE WHEN VP.EStatus = 1 THEN VP.VisitPlanID END) AS No_of_App_Pending,
            COUNT(CASE WHEN VP.Status = 4 AND VP.EStatus = 3 THEN VP.VisitPlanID END) AS No_of_Complete,
            COUNT(CASE WHEN VP.EStatus = 5 THEN VP.VisitPlanID END) AS No_of_Cancelled,
            MAX(VP.AppStatus) AS AnyStatus,
            MAX(A.AppStatusmeans) AS Status
        FROM sndVisitPlans VP
        LEFT JOIN sndUsers U ON U.UserID = VP.VisitUserID
        LEFT JOIN sndApprovals A 
            ON A.AppStatus = VP.AppStatus AND A.ApprovalTables = 'sndVisitPlans'
        WHERE VP.EStatus IN (1,4,5) AND VP.VEStatus IN (1,4,5)
        GROUP BY VP.VisitUserID, U.EmpName;
        ";
        $params = [];

    // === CASE 2: Mid-level approval users ===
    } elseif (in_array($UserID, [2051,2885,393,405,430,633,237,241,257,299,340,342,355])) {
        $sql = "
        SELECT 
            ROW_NUMBER() OVER (ORDER BY COUNT(CASE WHEN VP.EAppStatus = 1 THEN VP.VisitPlanID END) DESC) AS SL,
            VP.VisitUserID,
            U.EmpName AS VisitUserName,
            COUNT(CASE WHEN VP.EStatus = 1 THEN VP.VisitPlanID END) AS No_of_App_Pending,
            COUNT(CASE WHEN VP.EStatus = 3 THEN VP.VisitPlanID END) AS No_of_Complete,
            COUNT(CASE WHEN VP.EStatus = 5 THEN VP.VisitPlanID END) AS No_of_Cancelled,
            MAX(VP.AppStatus) AS AnyStatus,
            MAX(AP.AppStatusmeans) AS Status
        FROM sndVisitPlans VP
        LEFT JOIN sndUsers U ON U.UserID = VP.VisitUserID
        LEFT JOIN sndApprovals AP 
            ON AP.AppStatus = VP.EStatus 
            AND AP.ApprovalTables = 'sndVisitPlansEntry'
        WHERE VP.EStatus IN (1,3,5)
          AND VP.VEStatus = 1
          AND EXISTS (
              SELECT 1
              FROM sndUserRoleMapping URM
              JOIN sndApprovals A2 
                ON A2.RoleID = URM.RoleID
              WHERE URM.UserID = ?
                AND A2.ApprovalTables = 'sndVisitPlansEntry'
                AND A2.AppStatus = VP.EStatus
          )
          AND VP.UserID IN (
              SELECT UserID FROM sndUsers WHERE ReportingToUserID = ?
          )
    ...
    // === CASE 3: Lower-level multi-tier managers ===
    } elseif (in_array($UserID, [2051,344,345,354,472,258,716])) {
        $sql = "
        SELECT 
            ROW_NUMBER() OVER (ORDER BY COUNT(CASE WHEN VP.EAppStatus = 1 THEN VP.VisitPlanID END) DESC) AS SL,
            VP.VisitUserID,
    ...
    // === CASE 4: Special role (UserID 705) ===
        ...
        SELECT 
            ...
            VP.VisitUserID,
            U.EmpName AS VisitUserName,
            COUNT(CASE WHEN VP.EStatus = 3 THEN VP.VisitPlanID END) AS No_of_App_Pending,
            COUNT(CASE WHEN VP.EStatus = 4 THEN VP.VisitPlanID END) AS No_of_Complete,
            COUNT(CASE WHEN VP.EStatus = 5 THEN VP.VisitPlanID END) AS No_of_Cancelled,
            MAX(VP.AppStatus) AS AnyStatus,
            MAX(AP.AppStatusmeans) AS Status
        FROM sndVisitPlans VP
        LEFT JOIN sndUsers U ON U.UserID = VP.VisitUserID
        LEFT JOIN sndApprovals AP 
            ON AP.AppStatus = VP.AppStatus 
            AND AP.ApprovalTables = 'sndVisitPlans'
        WHERE VP.EStatus = 3
          AND VP.VEStatus = 1
          AND EXISTS (
              SELECT 1
        ...
        echo json_encode(["error" => "UserID not permitted or no matching rule."]);
        return;
    }
...
function get_VisitPlanApprovalSum($conn, $UserID) {
    // Define which user IDs should see the full summary (admin users)
    $adminUserIDs = [1, 2057, 1001]; // Replace with your actual admin user IDs
...
if (in_array($UserID, [2851,69,1718,1759])) {   
    $sql = "SELECT 
...
function get_VisitPlanApprovalnew($conn, $UserID) {
    // Define which user IDs should see the full summary (admin users)
    $adminUserIDs = [1759]; // Replace with your actual admin user IDs
...
from sndPartyMaster order by PartyName
        ";
        $params = []; // No parameter needed for InstitutionTypeID == 186
...
function get_institutionNameUsers($conn, $InstitutionTypeID, $UserID) {
    ...
    // Define which users can access all institutions
    $specialUserIDs = [1, 2, 3, 501]; // ✅ Add actual user IDs as needed

    // If user is in the allowed list, fetch all institutions
    if (in_array($UserID, $specialUserIDs)) {
        $sql = "
            SELECT 
                PartyID AS InstitutionID, 
                CONCAT(PartyName, ' - ', 
                    (SELECT ThanaName FROM sndRegionView WHERE AreaID = RegionID)
                ) AS InstitutionName
            FROM sndPartyMaster
            ORDER BY PartyName
        ";
    } else {
        // If you ever want to restrict for others, you can modify here
        $sql = "
            SELECT 
                PartyID AS InstitutionID, 
                CONCAT(PartyName, ' - ', 
...
function get_institutionNameUsers1($conn, $InstitutionTypeID, $UserID) {
    $sql = "";
    $params = [];
...
function get_all_institutions_page($conn, $UserID, $page, $limit) {
    $baseURL = "http://asianabpolymar.com/abpolymer/"; // Replace with your actual domain or base URL
...
function get_institution($conn, $institutionID) {
    ...
    if ($institution) {
        ...
        if (!empty($institution['InstitutionScanImagePath'])) {
            $baseUrl = "http://asianabpolymar.com/abpolymer/"; // Update with your actual base URL
...
function update_institutionAndroid($conn, $institutionID) {
    ...
    $fullImagePath = $imagePath ? "http://asianabpolymar.com/abpolymer/" . $imagePath : null;
...
function get_parties_users($conn, $UserID) {
    // Define SQL and params
    $sql = "";
    $partyParams = [];

    // If the user has full access (e.g., admin)
    if (in_array($UserID, [2879])) {
        $sql = "
        ...
        // For normal users — filter by mapped regions
        $sql = "
             SELECT PartyID, PartyName
            FROM sndPartyMaster
            ORDER BY PartyName
...
function get_partydetailsAndroid($conn, $partyID) {
    try {
        ...
        if ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
            ...
            $baseUrl = "http://asianabpolymar.com/abpolymer/uploads/party_files/";
            $imageFields = [
                'OwnerPicturePath', 
                'OwnerNIDDocumentPath', 
                'TradeLicenseDocumentPath', 
                ...
                if ($_FILES['PartyDocsPath']['error'][$i] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($_FILES['PartyDocsPath']['tmp_name'][$i], $targetFilePath)) {
                        ...
                        $uploadedFiles[] = [
                            ...
                            "PartyDocsPath" => "http://asianabpolymar.com/abpolymer/uploads/party_files/" . $encodedPartyName . "/" . $fileName
                        ];
                    } else {
...
// Change function name from get_all_regions to get_regions
...
function get_all_regions($conn) {
    ...
    if ($stmt === false) {
        http_response_code(500);
        echo json_encode(["error" => "Error fetching regions", "details" => sqlsrv_errors()], JSON_UNESCAPED_UNICODE);
        exit;
    }

    $regions = [];
    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        // Convert all to UTF-8 to avoid malformed JSON
...
// Fetch all regions
function get_all_regions1($conn) {
    $sql = "SELECT 
	    Area.RegionID AS AreaID,
...
function get_BDExpReqs_users($conn, $UserID) {
    ...
    // ✅ define which user(s) can view all data
    $adminUsers = [2879]; // add more IDs if needed

    if (in_array($UserID, $adminUsers)) { 
        // ✅ Admin user — can see all BDExpReq records
        $sql = "SELECT 
            ROW_NUMBER() OVER (ORDER BY sndBDExpReq.BDExpReqID) AS SL, 
            sndBDExpReq.BDExpReqID,
            sndBDExpReq.BDExpReqNo,
            CONCAT(sndBDExpReq.BDExpReqNo,' -', 'Tk. ', FORMAT(ISNULL(sndBDExpReq.TotalAmount, 0), 'N2')) as BDExpInfo1,
            CONCAT(convert(char(10),sndBDExpReq.BDExpReqDate,105),' - ', 
                (select Statusmeans from sndStatus where sndStatus.status = sndBDExpReq.status and statustables = 'sndBDExpReq')) as BDExpInfo2,    
            CASE 
                WHEN sndBDExpReq.InstitutionID IS NULL THEN 
                    (SELECT PartyName FROM sndPartyMaster WHERE sndPartyMaster.PartyID = sndBDExpReq.PartyID)
                ELSE
                    (SELECT InstitutionName FROM sndInstitutions WHERE InstitutionID = sndBDExpReq.InstitutionID)
            END AS BDExpInfo3,
            sndBDExpReq.BDExpUserID,
            CONVERT(char(11), sndBDExpReq.BDExpReqDate, 120) AS BDExpReqDate,
            sndCategorydata.id AS InstitutionTypeID,
            ...
            CASE 
                WHEN EXISTS (
                    SELECT 1 FROM sndBDExpReqDetails WHERE sndBDExpReqDetails.BDExpReqID = sndBDExpReq.BDExpReqID
                ) THEN 'True'
                ELSE 'False'
            END AS DetailsStatus
        FROM 
            dbo.sndBDExpReq
        LEFT JOIN sndCategorydata
            ON sndBDExpReq.InstitutionTypeID = sndCategorydata.id
            AND sndCategorydata.CategoryType = 'institution-type'
        LEFT JOIN sndInstitutions
        ...
        // ✅ Normal user — only their own + reporting hierarchy
        $sql = "SELECT 
            ROW_NUMBER() OVER (ORDER BY sndBDExpReq.BDExpReqID) AS SL, 
            sndBDExpReq.BDExpReqID,
            sndBDExpReq.BDExpReqNo,
    ...
    if ($stmt === false) {
        echo json_encode(["error" => "Error fetching BDExpReqs: " . print_r(sqlsrv_errors(), true)]);
        return;
    }
...
WHERE 
    ...
    if (!empty($userDetails['Userpicture'])) {
        $userDetails['Userpicture'] = 'http://asianabpolymar.com/abpolymer/' . $userDetails['Userpicture'];
    }
...
function get_ppreceiptall($conn,$ProductReceiptID) {
    ...
    $baseURL = "http://asianabpolymar.com/abpolymer/uploads/ChallanCopyPath/";
...
function get_preceipt($conn, $ProductReceiptID) {
    ...
    if ($receipt) {
        ...
        if (!empty($receipt['ChallanCopyPath']) && file_exists($receipt['ChallanCopyPath'])) {
            ...
            $baseUrl = "http://asianabpolymar.com/abpolymer/";
...
function create_preceipt($conn) {
    try {
        ...
        if (isset($_FILES['ChallanCopyPath']) && $_FILES['ChallanCopyPath']['error'] === UPLOAD_ERR_OK) {
            ...
            $fileURL = "http://asianabpolymar.com/abpolymer/uploads/ChallanCopyPath/" . $fileName;
        } else {
...
function create_ppreceiptall433($conn) {
    ...
    if (isset($_FILES['ChallanCopyPath']) && $_FILES['ChallanCopyPath']['error'] === UPLOAD_ERR_OK) {
        ...
        // Set the relative path to save in the database
        $challanCopyPath = 'http://asianabpolymar.com/abpolymer/uploads/ChallanCopyPath/' . $fileName;
    }
