...
SELECT 
    ...
    dcm.Status,
    dcm.UserID,
    (select EmpName from sndUsers where sndUsers.UserID = dcm.UserID) as UserName, 
    ...
    -- Calculate Net_Amount (TotalAmount + NetAmount from sndInvoiceDetails + Expenses)
	 ...
	 (SELECT SUM(Amount) 
     FROM sndInvoiceDetails 
     WHERE InvoiceID = dcm.InvoiceID)+(SELECT SUM(Amount) 
...
function create_BDExpReqMaster($conn) {
    ...
    if ($stmt === false) {
        echo json_encode(["error" => "Error creating BDExpReq: " . print_r(sqlsrv_errors(), true)]);
        return;
    }
    ...
    $sql = "
        SELECT 
         ROW_NUMBER() OVER (ORDER BY MRID) AS SL, 
            MRID,
    CONCAT(MRNo,' -', 'Tk. ', AmountReceived) as MRInfo1,
...
function get_moneyreceipts($conn) {
    ...
    convert(char(11),MRDate,120) as MRDate, AmountReceived, dbo.fnNumberToWords(AmountReceived) as InWord,PaymentMethodID,
...
function update_moneyreceipt($conn, $MRID) {
    ...
    $sql = "UPDATE sndMoneyReceipt 
            SET PartyID = ?, MRDate = ?, AmountReceived = ?, InWord = ?, PaymentMethodID = ?, PaymentMethodDetailsID = ?, AccName = ?, AccNumber = ?, ChequeNumber = ?, Remarks = ?, ReceivedByUserID = ?, ModifiedAt = GETDATE() 
            WHERE MRID = ?";
    $params = [
        $data['PartyID'], 
        $data['MRDate'], 
...
function get_BDExpReqComplete($conn, $UserID) {
    ...
    $sql = "
       SELECT 
        ROW_NUMBER() OVER (ORDER BY BDExpReqID) AS SL, 
    BDExpReqID,
     CONCAT(BDExpReqNo,' -', TotalAmount) as BDExpInfo1,
...
function get_orderforchallan($conn, $SalesOrderID) {
    ...
    $sqlDetails = "SELECT 
                       ...
                       Price,
                       CONCAT(Quantity,' Pcs', ' @ Tk. ', Price, ' = ', Quantity*Price) AS RateValue,
                       (select sum(Quantity) from sndStockInOut where sndStockInOut.FinancialYearID = sndSalesOrderDetails.FinancialYearID
					   and sndStockInOut.ProductCategoryID = sndSalesOrderDetails.ProductCategoryID and
					   sndStockInOut.ProductID = sndSalesOrderDetails.ProductID and sndStockInOut.TransactionType = 'Stock-In') -
					   (select sum(Quantity) from sndStockInOut where sndStockInOut.FinancialYearID = sndSalesOrderDetails.FinancialYearID
					   and sndStockInOut.ProductCategoryID = sndSalesOrderDetails.ProductCategoryID and
					   sndStockInOut.ProductID = sndSalesOrderDetails.ProductID and sndStockInOut.TransactionType = 'Stock-Out') as AvailableQty
                   FROM sndSalesOrderDetails
                   WHERE SalesOrderID = ?";
...
function get_salesorders($conn) {
    ...
    SalesOrderID
      ...
      ,CONCAT(TotalAmount, ' - ', (select Statusmeans from sndStatus where status = sndSalesOrders.status and statustables = 'sndSalesOrders')) as SoInfo3
...
then 'Authrized' else 'Not Authorized' end as UserApprovalSatus
  FROM [dbo].[sndSalesOrders] 
  where OrderTypeID = 1 and Status =1 or OrderTypeID = 1 and Status =2
  order by SalesOrderID desc";
...
then 'Authrized' else 'Not Authorized' end as UserApprovalSatus
  FROM [dbo].[sndSalesOrders] 
  where OrderTypeID = 1 and  Status =3 
  order by SalesOrderID desc";
...
then 'Authrized' else 'Not Authorized' end as UserApprovalSatus
  FROM [dbo].[sndSalesOrders] 
  where OrderTypeID = 1 and  Status =4
  order by SalesOrderID desc";
...
then 'Authrized' else 'Not Authorized' end as UserApprovalSatus
  FROM [dbo].[sndSalesOrders] 
  where OrderTypeID = 1 and  Status =5
  order by SalesOrderID desc";
...
function get_salesordersInvoice($conn) {
  ...
  OrderTypeID = 1 and  sndSalesOrders.Status =3 and sndSalesOrders.InvoiceStatus =0  and challanstatus =1
...
then 'Authrized' else 'Not Authorized' end as UserApprovalSatus
  FROM [dbo].[sndSalesOrders] 
  where OrderTypeID = 2 and Status =1 or  OrderTypeID = 2 and Status =2
  order by SalesOrderID desc";
...
then 'Authrized' else 'Not Authorized' end as UserApprovalSatus
  FROM [dbo].[sndSalesOrders] 
  where OrderTypeID = 2 and  Status =3
  order by SalesOrderID desc";
...
then 'Authrized' else 'Not Authorized' end as UserApprovalSatus
  FROM [dbo].[sndSalesOrders] 
  where OrderTypeID = 2 and Status =4 
  order by SalesOrderID desc";
...
then 'Authrized' else 'Not Authorized' end as UserApprovalSatus
  FROM [dbo].[sndSalesOrders] 
  where OrderTypeID = 2 and Status =5
  order by SalesOrderID desc";
...
function create_preturndetails($conn) {
    ...
    // Update the total amount in sndProductReturn
    $updateTotalSQL = "UPDATE sndProductReturn
                       SET TotalAmount = (
                           SELECT SUM(Quantity * Rate) AS subtotal
                           FROM sndProductReturnDetails
                           WHERE sndProductReturnDetails.ProductReturnID = sndProductReturn.ProductReturnID
                       )
                       WHERE sndProductReturn.ProductReturnID = ?";
    $paramsUpdateTotal = [$input['ProductReturnID']];
    ...
    // Return success message
    echo json_encode([
        "message" => "Product Return detail created and total amount updated successfully",
...
function get_VisitPlanApprovalSum($conn, $UserID) {
    ...
    (SELECT DISTINCT AppStatusmeans 
     FROM sndApprovals 
     WHERE AppStatus = sndVisitPlans.AppStatus 
...
function get_visit_entryforApproval($conn, $VisitPlanID) {
    try {
        ...
        $visitPlanSql = "SELECT 
                ...
                CONVERT(CHAR(19), CreatedAt, 120) AS CreatedAt,
                CONVERT(CHAR(19), ModifiedAt, 120) AS ModifiedAt,
                CheckInTime, CheckOutTime, Latitude, 
                Longitude, VisitEntryDate, VEStatus
            FROM sndVisitPlans  
            WHERE  VisitPlanID = ?";
...
function get_VisitPlanEntryApproval($conn, $UserID) {
    $sql = "SELECT 
     ROW_NUMBER() OVER (ORDER BY VisitPlanID) AS SL, 
           ...
           ,convert(char(19),ModifiedAt,120) as ModifiedAt 
	FROM sndVisitPlans 
    WHERE UserID = ? and Status =2 and EStatus in (1,2,3)";
    $params = [$UserID];
    $stmt = sqlsrv_query($conn, $sql, $params);
...
function get_VisitPlanEntryApprovalComplete($conn, $UserID) {
    $sql = "SELECT 
     ROW_NUMBER() OVER (ORDER BY VisitPlanID) AS SL, 
           ...
           ,convert(char(19),ModifiedAt,120) as ModifiedAt 
	FROM sndVisitPlans 
    WHERE UserID = ? and Status =2 and EStatus =4";
    $params = [$UserID];
    $stmt = sqlsrv_query($conn, $sql, $params);
...
function get_VisitPlanEntryApprovalReject($conn, $UserID) {
    $sql = "SELECT 
     ROW_NUMBER() OVER (ORDER BY VisitPlanID) AS SL, 
           ...
           ,convert(char(19),ModifiedAt,120) as ModifiedAt 
	FROM sndVisitPlans 
    WHERE UserID = ? and Status =2 and EStatus =5";
    $params = [$UserID];
    $stmt = sqlsrv_query($conn, $sql, $params);
...
function get_VisitPlanEntryApprovalCancelled($conn, $UserID) {
    $sql = "SELECT 
     ROW_NUMBER() OVER (ORDER BY VisitPlanID) AS SL, 
           ...
           ,convert(char(19),ModifiedAt,120) as ModifiedAt 
	FROM sndVisitPlans 
    WHERE UserID = ? and Status =2 and EStatus =6";
    $params = [$UserID];
    $stmt = sqlsrv_query($conn, $sql, $params);
...
function get_VisitPlanCompleteList($conn, $UserID) {
    $sql = "SELECT 
     ROW_NUMBER() OVER (ORDER BY VisitPlanID) AS SL, 
           ...
           ,convert(char(19),ModifiedAt,120) as ModifiedAt 
	FROM sndVisitPlans 
    WHERE UserID = ? and Status =2  and VEStatus = 0
...
function get_VisitPlanCompleteEntryList($conn, $UserID) {
    ...
    ROW_NUMBER() OVER (ORDER BY VisitPlanID) AS SL, 
            ...
            ,CONCAT(convert(char(11),VisitPlanDate,105),' -', (SELECT CategoryName 
            FROM sndCategorydata 
            WHERE sndCategorydata.id = sndVisitPlans.InstitutionTypeID 
              AND CategoryType = 'institution-type') ) as VPInfo2
           ...
           ,convert(char(19),ModifiedAt,120) as ModifiedAt 
	FROM sndVisitPlans 
    WHERE UserID = ? and Status =2  and VEStatus = 1
    order by VisitPlanID desc";
    $params = [$UserID];
    $stmt = sqlsrv_query($conn, $sql, $params);
...
function create_visit_plan($conn) {
    ...
    // Correct field names in validation
    if (!isset($input['VisitPlanNo']) || !isset($input['VisitPlanDate']) || !isset($input['InstitutionID'])) {
        echo json_encode(["error" => "Missing required fields"]);
        return;
    }
    ...
    $params = [
        $input['VisitPlanNo'],                  // Correct key for VisitPlanNo
        $input['VisitPlanDate'],                // Correct key for VisitPlanDate
        $input['InstitutionTypeID'],           // Correct key for InstitutionTypeID
        $input['InstitutionID'],               // Correct key for InstitutionID
        $input['PurposeID'] ?? null,           // Optional key with null default
        $input['VisitUserID'] ?? null,         // Optional key with null default
        $input['UserID']                       // Correct key for UserID
    ];
            ...
            SELECT 
                ...
                CASE 
                    WHEN (SELECT COUNT(*) 
                          FROM sndBDExpReq 
                          WHERE 
                    sndBDExpReq.InstitutionID = sndVisitPlans.InstitutionID 
                    AND sndBDExpReq.BDExpUserID = sndVisitPlans.VisitUserID) = 0 
                    THEN 'False' 
                    ELSE 'True' 
                END AS DetailsStatus            
            FROM sndVisitPlans 
...
FROM dbo.sndVisitPlanTADADetails vpt
LEFT JOIN sndThansportMedia tm 
    ON vpt.ThansportMediaID = tm.ThansportMediaID where
                VisitPlanID = ?";
...
function update_visit_plan($conn, $VisitPlanID) {
    ...
    // Check for required fields using correct keys
    if (!isset($input['VisitPlanDate']) || !isset($input['UserID']) || !isset($input['InstitutionID'])) {
        echo json_encode(["error" => "Missing required fields"]);
        return;
    }
...
function get_institutionTeacher($conn, $InstitutionID) {
    $sql = "SELECT InstitutionDetailsID, TeacherName, Designation, ContactPhone FROM sndInstitutionDetails WHERE InstitutionID = ?";
...
function get_visit_plan_Teacher_TADA($conn, $VisitPlanID) {
    try {
        ...
        // Query to fetch TeacherBooks details
        $sqlTeacherBooks = "
            SELECT 
                 ROW_NUMBER() OVER (ORDER BY VisitPlanID) as SL,
                VisitPlanID,
...
WHERE 
    mmrv.UserID = ? order by pm.PartyName;";
	$partyParams = [$UserID];
...
function get_user_statistics($conn, $UserID) {
    try {
        ...
        $expenseQueries = [
            ...
            'TA_Expense' => [
                "SELECT FORMAT(ISNULL(SUM(Amount), 0), 'N2') AS TA_Expense 
                 FROM sndVisitPlanTADADetails td
                 JOIN sndVisitPlans vp ON td.VisitPlanID = vp.VisitPlanID
                 WHERE  td.TADACategoryID = 21 AND vp.UserID = ?"
            ],
            'DA_Expense' => [
                "SELECT FORMAT(ISNULL(SUM(Amount), 0), 'N2') AS DA_Expense 
                 FROM sndVisitPlanTADADetails td
                 JOIN sndVisitPlans vp ON td.VisitPlanID = vp.VisitPlanID
                 WHERE td.TADACategoryID = 22 AND vp.UserID = ?"
            ],
            'Others_Expense' => [
                "SELECT FORMAT(ISNULL(SUM(Amount), 0), 'N2') AS Others_Expense 
                 FROM sndVisitPlanTADADetails td
                 JOIN sndVisitPlans vp ON td.VisitPlanID = vp.VisitPlanID
                 WHERE vp.PurposeID NOT IN (21, 22) AND vp.UserID = ?"
            ],
        ];
...
(SELECT  Name
 ...
 (SELECT  CategoryName
              ...
              WHERE   (ProductID = sio.ProductID)) AS ProductName, sum(sio.Quantity) as TotalDemand, sum(sio.Rate) as TotalDonation,
			  CONCAT('Demand Qty-', FORMAT(ISNULL(sum(sio.Quantity), 0), 'N2'), ', BD Exp. Tk.- ', sum(sio.Rate)) as ProductsValue
 from sndStockInOut sio, sndSalesOrderDetails SOD, sndSalesOrders so, sndPartyMaster pm
 where so.SalesOrderID = sod.SalesOrderID 
 and pm.PartyID = so.PartyID
 and sio.FinancialYearID = sod.FinancialYearID 
 and sio.FinancialYearID = (select id from CmnTransactionalYears where CmnCompanyId = 2 and ModuleId = 06 and Status =1 and right(Name,2) = right(convert(char(10),getdate(),105),2))
 and sio.ProductCategoryID = sod.ProductCategoryID 
  and sio.ProductID = sod.ProductID 
 and sio.TransactionType = 'Demand-In' and pm.DistrictID = sio.DistrictID
 and so.OrderTypeID = 1 and right(sio.VoucherReference,2) = right(convert(char(10),getdate(),105),2)
 and so.SalesOrderID = ?
 group by sio.FinancialYearID, sio.ProductCategoryID,sio.ProductID";
    $params = [$SalesOrderID];
...
Function get_all_financialyears($conn) {
    $sql = "select id, name, OpeningDate,ClosingDate, YearClosingStatus from CmnTransactionalYears where ModuleId = '06' and CmnCompanyId = '4' AND Status = 1
";
    $stmt = sqlsrv_query($conn, $sql);
...
function get_BDExpReqs($conn) {
    ...
    sndBDExpReq.BDExpReqID,
    sndBDExpReq.BDExpReqNo,
    CONCAT(sndBDExpReq.BDExpReqNo,' -', 'Tk. ', sndBDExpReq.TotalAmount) as BDExpInfo1,
    CONCAT(convert(char(10),sndBDExpReq.BDExpReqDate,105),' - ', (select Statusmeans from sndStatus where sndStatus.status = sndBDExpReq.status and statustables = 'sndBDExpReq')) as BDExpInfo2,    
     sndInstitutions.InstitutionName as BDExpInfo3,
    sndBDExpReq.BDExpUserID,
    CONVERT(char(11), sndBDExpReq.BDExpReqDate, 120) AS BDExpReqDate,
    sndBDExpReq.InstitutionTypeID,
    sndCategorydata.id AS InstitutionTypeID,
...
function get_BDExpReqAll($conn, $BDExpReqID) {
    ...
    $sqlMain = "
        SELECT 
            sndBDExpReq.BDExpReqID,
             CONCAT(sndBDExpReq.BDExpReqNo,' -', sndBDExpReq.TotalAmount) as BDInfo1,
            CONVERT(char(10), sndBDExpReq.BDExpReqDate, 105) as BDInfo2,
           sndInstitutions.InstitutionName as BDInfo3,
            sndBDExpReq.BDExpReqNo,
            sndBDExpReq.BDExpUserID,
            CONVERT(char(11), sndBDExpReq.BDExpReqDate, 120) AS BDExpReqDate,
            sndBDExpReq.InstitutionTypeID,
...
function get_all_ReportUser($conn, $DesignationID) {
    try {
        ...
        if ($stmt === false) {
            throw new Exception("Failed to prepare SQL: " . print_r(sqlsrv_errors(), true));
        }

        if (!sqlsrv_execute($stmt)) {
            throw new Exception("Failed to execute SQL: " . print_r(sqlsrv_errors(), true));
        }
        ...
        while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
            $results[] = [
                ...
                'ReportUsers' => $row['reportUsers'] ?? null
            ];
        }

        sqlsrv_free_stmt($stmt);

        // Always return JSON-safe array
        return [
            'status' => 'success',
            'data' => $results
        ];
