...
// CORS function to set all headers consistently
function cors() {
    header('Content-Type: application/json');
...
function get_InvoiceOrderDetails($conn, $InvoiceID) {
    ...
    $sqlMaster = "SELECT 
        dcm.InvoiceID,
        dcm.InvoiceNo,
        CONVERT(CHAR(11), dcm.InvoiceDate, 120) AS ChallanDate,
        so.PartyID,
        pm.PartyName,
        so.SpecimenUserID,
...
function Create_InvoiceAll($conn) { 
    ...
    try {
        ...
        $sqlMaster = "INSERT INTO sndInvoiceMaster (InvoiceNo, InvoiceDate, ChallanID, TotalAmount, UserID, Status, CreatedAt) 
                      OUTPUT INSERTED.InvoiceID VALUES (?, ?, ?, ?, ?, ?, GETDATE())";
        ...
        $sqlDetails = "INSERT INTO sndInvoiceDetails (InvoiceID, FinancialYearID, ProductCategoryID, ProductID, Quantity, UnitPrice, CreatedAt) 
                       VALUES (?, ?, ?, ?, ?, ?, GETDATE())";
...
function create_BDExpReqDetails($conn) {
    ...
    $requiredFields = ['BDExpReqID', 'TeacherName', 'Designation', 'ContactPhone', 'BooksGroupID', 'ProductID', 'StudentsCount', 'DonationAmount'];
    ...
    try {
        ...
        // Update the TotalAmount in the sndBDExpReq table
...
function get_moneyreceipts($conn) {
    ...
    convert(char(11),MRDate,120) as MRDate, AmountReceived,InWord,PaymentMethodID,
	...
	and sndPaymentMethodDetails.PaymentMethodDetailsID = sndMoneyReceipt.PaymentMethodDetailsID) as  PaymentMethodDetails, 
    ...
    $sql = "
        SELECT 
            ...
            (SELECT PMDName 
             ...
             WHERE sndPaymentMethodDetails.PaymentMethodDetailsID = sndMoneyReceipt.PaymentMethodDetailsID) AS PaymentMethodDetails, 
...
function get_specimenordersInvoice($conn) {
  ...
  where sndSalesOrders.SalesOrderID = sndDeliveryChallanMaster.SalesOrderID and
  OrderTypeID = 2 and  sndSalesOrders.Status =3 and sndSalesOrders.InvoiceStatus =0 and ChallanStatus =0
  order by sndSalesOrders.SalesOrderID desc";
...
function get_visit_entryall($conn, $VisitPlanID) {
    try {
        ...
        $detailsSql = "SELECT 
                           ...
                           FinancialYearID, ProductCategoryID, ProductID, StudentNo, 
        ...
        $tadaSql = "SELECT 
                        sndVisitPlanTADADetailsID, TADACategoryID, Amount, CreatedAt 
                    FROM sndVisitPlanTADADetails 
                    WHERE VisitPlanID = ?";
        ...
        if (isset($input['TADADetails']) && is_array($input['TADADetails'])) {
            foreach ($input['TADADetails'] as $tadaDetail) {
                ...
                $tadaSql = "INSERT INTO dbo.sndVisitPlanTADADetails
                            (VisitPlanID, TADACategoryID, Amount, CreatedAt)
                            VALUES (?, ?, ?, GETDATE())";
...
function get_visit_plan_TADA_details($conn, $VisitPlanID) {
    try {
        ...
        $sql = "SELECT 
                     ROW_NUMBER() OVER (ORDER BY sndVisitPlanTADADetailsID) AS SL,
                    sndVisitPlanTADADetailsID, 
                    TADACategoryID, 
                    Amount, 
                    CreatedAt 
                FROM dbo.sndVisitPlanTADADetails 
                WHERE VisitPlanID = ?";
    ...
    try {
        ...
        $sqlTADA = "
            SELECT 
                ...
                sndVisitPlanTADADetailsID, 
                TADACategoryID, 
                (select CategoryName from sndCategorydata where CategoryType = 'TA/DA Allowance' and sndCategorydata.id = sndVisitPlanTADADetails.TADACategoryID)
                as TADACategory,
                Amount, 
                CreatedAt 
            FROM dbo.sndVisitPlanTADADetails 
...
function get_Notification($conn, $NotificationID) {
    $sql = "select NotificationsID, sndNotificationsID, NotificationTitle ,NotificationStatusMeans  from sndNotificationsGenerate where NotificationsID = ?";
...
function get_salesordersAutorizationMIS($conn, $SalesOrderNo) {
    //$sql = "SELECT avg(Rate) as Rate FROM sndProductReceipt WHERE FinancialYearID = ? AND ProductID = ?";
	$sql = "select 
    ROW_NUMBER() OVER (PARTITION BY financialyear ORDER BY financialyear) AS SL,
    FinancialYear, ProductName, CONCAT(sum(quantity), ' Pcs', ' @ Tk. ', FORMAT(ISNULL(avg(price), 0), 'N2'), ' = ', FORMAT(ISNULL(sum(quantity)*avg(price), 0), 'N2')) as ProductsValue  from sndSalesOrderDemandLastYear
where partyid = ?
group by financialyear, productname";
    $params = [$SalesOrderNo];
...
function get_salesordersAutorizationMISReturn($conn, $SalesOrderNo) {
    //$sql = "SELECT avg(Rate) as Rate FROM sndProductReceipt WHERE FinancialYearID = ? AND ProductID = ?";
	$sql = "select 
    ROW_NUMBER() OVER (PARTITION BY financialyear ORDER BY financialyear) AS SL,
    FinancialYear, ProductName, CONCAT(sum(quantity), ' Pcs', ' @ Tk. ', FORMAT(ISNULL(avg(price), 0), 'N2'), ' = ', FORMAT(ISNULL(sum(quantity)*avg(price), 0), 'N2')) as ProductsValue  from sndSalesOrderDemandLastYear
...
function get_BDExpReqs($conn) {
    ...
    sndBDExpReq.BDExpReqID,
    sndBDExpReq.BDExpReqNo,
     sndBDExpReq.BDExpReqNo as BDExpInfo1,
     convert(char(11),sndBDExpReq.BDExpReqDate,120) as BDExpInfo2,
...
function get_BDExpReqAll($conn, $BDExpReqID) {
    ...
    $sqlDetails = "
        SELECT 
            ...
            (SELECT ProductName 
             FROM sndProducts 
             WHERE sndProducts.ProductID = sndBDExpReqDetails.ProductID) AS BDEDetails2,
            [StudentsCount] AS BDEDetails3,
            [TeacherName],
            [Designation],
            [ContactPhone],
            [FinancialYearID],
...
function generate_new_invoice_number($conn) {
    ...
    // Check if a previous record exists and extract its numeric part
    if ($row = sqlsrv_fetch_array($stmtLastOrder, SQLSRV_FETCH_ASSOC)) {
        if (isset($row['InvoiceNo']) && preg_match('/IN-(\d+)\/\d{2}-\d{2}/', $row['InvoiceNo'], $matches)) {
            $lastNumber = (int)$matches[1];
        }
    }
